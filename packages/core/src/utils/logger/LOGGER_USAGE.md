...
```typescript
// DEBUG: 详细的内部状态、调试信息
logger.debug('ChunkManager', '分片切割完成', { 
  totalChunks: 10, 
...
// INFO: 关键业务流程节点
logger.info('UploadFile', '文件上传成功', fileName);
logger.info('ChunkManager', '所有分片合并完成');

// WARN: 可恢复的异常、降级处理
logger.warn('ChunkManager', 'IndexedDB 更新失败，使用备用方案');
logger.warn('UploadFile', '缺少 onMerge 回调，跳过合并步骤');

// ERROR: 导致功能失败的严重错误
logger.error('Uploader', '网络请求失败', error);
logger.error('ChunkManager', '分片上传超时', { chunkIndex, timeout });
```
...
try {
  ...
  logger.error('ChunkManager', '分片上传失败', error);
}

// ❌ 不推荐
logger.error('ChunkManager', `分片上传失败: ${error.message}`);
```
...
// ✅ 新代码
logger.info('ChunkManager', '开始上传');
logger.warn('ChunkManager', '更新失败:', error);
...
- [ChunkManager 断点续传规范](./CHUNK_MANAGER_DB_SPEC.md)
