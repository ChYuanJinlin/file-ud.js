...
```typescript
public setFiles(files: IFile[]): void
```
...
| `url` | `string` | ❌ | 文件访问 URL |
| `size` | `number` | ❌ | 文件原始大小（字节），来自服务端或原生 File 对象 |
...
| `fileHandle` | `FileSystemFileHandle` | ❌ | 流式保存句柄（下载场景） |

### 分片回显字段

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `totalChunks` | `number` | 分片回显时 ✅ | 总分片数；存在该字段时会初始化分片回显状态 |
| `completedChunks` | `number` | ❌ | 已完成分片数 |
| `chunkIndexes` | `number[]` | ❌ | 已上传分片索引数组；不传时会按 `completedChunks` 假设前 N 个分片已完成 |
| `fileHash` | `string` | ❌ | 文件哈希，用于秒传、断点续传、IndexedDB 文件缓存 |
| `uploadId` | `string` | ❌ | 服务端上传任务 ID |
...
async function importFilesFromCSV(csvContent: string) {
  ...
  const files = rows.map(row => {
    const [name, url, sizeBytes] = row.split(',');
    const size = Number(sizeBytes.trim());
...
**原因**：[UploadFile](https://github.com/ChYuanJinlin/file-ud.js/blob/main/packages/core/src/uploader/UploadFile.ts) 类依赖 File 对象进行后续操作（如重新上传、获取文件大小等）。
...
**当前行为**：[setFiles](https://github.com/ChYuanJinlin/file-ud.js/blob/main/packages/core/src/uploader/index.ts) 会清空现有文件。
...
// 在模板中区分
<div v-for="file in files" :key="file.fileId">
  <span v-if="file.metadata?.isRestored">📁 已存在</span>
...
async function loadAndDisplayFiles() {
  try {
    ...
    const files = data.map(item => ({
      ...
      percent: 100,
      status: "success" as const,
      formatSize: formatFileSize(item.size)
    }));
...
- [clearFiles()](https://github.com/ChYuanJinlin/file-ud.js/blob/main/packages/core/src/uploader/index.ts) - 清空文件列表
- [onUpdate](https://github.com/ChYuanJinlin/file-ud.js/blob/main/packages/core/src/types/index.ts) - 文件列表更新回调
